...
(defun compile-file-request (opts)
  ...
  (case (xrepl-protocol-types:get-field-any opts '(file path))
    ('undefined
    ...
    (file
     (let* ((base (maps:put #"op" #"compile_file"
                           (xrepl-protocol-types:put-aliased
                            #m() 'file 'path
                            (xrepl-protocol-types:ensure-binary file))))
            (with-options (case (xrepl-protocol-types:get-field-any opts '(options compile_options))
                           ('undefined base)
                           (opts-val (xrepl-protocol-types:put-aliased
...
(defun compile-project-request (opts)
  ...
  (let* ((base (maps:put #"op" #"compile_project" #m()))
         (with-path (case (xrepl-protocol-types:get-field opts 'path 'undefined)
                     ('undefined base)
                     (p (maps:put #"path"
                                 (xrepl-protocol-types:ensure-binary p)
                                 base))))
         (with-clean (case (xrepl-protocol-types:get-field opts 'clean 'undefined)
                      ('undefined with-path)
                      (c (maps:put #"clean" c with-path))))
         (with-profile (case (xrepl-protocol-types:get-field-any opts '(profile build_profile))
                        ('undefined with-clean)
                        (prof (xrepl-protocol-types:put-aliased
                              with-clean 'profile 'build_profile
                              (xrepl-protocol-types:ensure-binary prof)))))
...
(defun lint-request (opts)
  ...
  (let* ((base (maps:put #"op" #"lint" #m()))
         (with-file (case (xrepl-protocol-types:get-field-any opts '(file path))
                     ('undefined base)
                     (f (xrepl-protocol-types:put-aliased
                        base 'file 'path
                        (xrepl-protocol-types:ensure-binary f)))))
         (with-code (case (xrepl-protocol-types:get-field opts 'code 'undefined)
                     ('undefined with-file)
                     (c (maps:put #"code"
                                 (xrepl-protocol-types:ensure-binary c)
                                 with-file))))
         (with-linters (case (xrepl-protocol-types:get-field opts 'linters 'undefined)
                        ('undefined with-code)
                        (ls (maps:put #"linters"
                                     (lists:map #'xrepl-protocol-types:ensure-binary/1 ls)
                                     with-code))))
         (with-session (xrepl-protocol-types:maybe-put-aliased
...
(defun dependencies-request (opts)
  ...
  (let* ((base (maps:put #"op" #"dependencies" #m()))
         ...
         (with-type (case (xrepl-protocol-types:get-field opts 'type 'undefined)
                     ('undefined with-path)
                     (t (maps:put #"type"
                                 (xrepl-protocol-types:ensure-binary t)
                                 with-path))))
         (with-session (xrepl-protocol-types:maybe-put-aliased
...
(defun build-request (opts)
  ...
  (let* ((base (maps:put #"op" #"build" #m()))
         (with-task (case (xrepl-protocol-types:get-field-any opts '(task target))
                     ('undefined base)
                     (t (xrepl-protocol-types:put-aliased
                        base 'task 'target
                        (xrepl-protocol-types:ensure-binary t)))))
         (with-args (case (xrepl-protocol-types:get-field opts 'args 'undefined)
                     ('undefined with-task)
                     (a (maps:put #"args"
                                 (lists:map #'xrepl-protocol-types:ensure-binary/1 a)
                                 with-task))))
         (with-clean (case (xrepl-protocol-types:get-field opts 'clean 'undefined)
                      ('undefined with-args)
  ...
  (let ((op (xrepl-protocol-types:get-field message 'op)))
    (cond
      ...
      ((or (== op #"compile_file") (== op 'compile_file))
       (let ((file (xrepl-protocol-types:get-field-any message '(file path)))
             (options (xrepl-protocol-types:get-field-any message '(options compile_options)))
      ...
      ((or (== op #"compile_project") (== op 'compile_project))
       (let ((path (xrepl-protocol-types:get-field message 'path))
             (clean (xrepl-protocol-types:get-field message 'clean))
             (profile (xrepl-protocol-types:get-field-any message '(profile build_profile)))
             (options (xrepl-protocol-types:get-field message 'options))
             (session (xrepl-protocol-types:get-field message 'session)))
      ...
      ((or (== op #"lint") (== op 'lint))
       (let ((file (xrepl-protocol-types:get-field-any message '(file path)))
             ...
             (linters (xrepl-protocol-types:get-field message 'linters))
             (session (xrepl-protocol-types:get-field message 'session)))
      ...
      ((or (== op #"dependencies") (== op 'dependencies))
       (let ((path (xrepl-protocol-types:get-field message 'path))
             (type (xrepl-protocol-types:get-field message 'type))
             (session (xrepl-protocol-types:get-field message 'session)))
      ...
      ((or (== op #"build") (== op 'build))
       (let ((task (xrepl-protocol-types:get-field-any message '(task target)))
             (args (xrepl-protocol-types:get-field message 'args))
             (clean (xrepl-protocol-types:get-field message 'clean))
             (session (xrepl-protocol-types:get-field message 'session)))
...
(defun parse-response (message op)
  ...
  (let ((status (xrepl-protocol-types:get-field message 'status)))
    (cond
      ...
      ;; lint response
      ((or (== op #"lint") (== op 'lint))
       (let ((issues (xrepl-protocol-types:get-field message 'issues)))
         (tuple 'ok (maps:put #"status" status
                             (maps:put #"issues" issues #m())))))

      ;; dependencies response
      ((or (== op #"dependencies") (== op 'dependencies))
       (let ((deps (xrepl-protocol-types:get-field message 'dependencies)))
         (tuple 'ok (maps:put #"status" status
                             (maps:put #"dependencies" deps #m())))))
