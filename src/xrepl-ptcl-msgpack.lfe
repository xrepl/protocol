  ...
  (case (encode term)
    (`#(ok ,msgpack-data)
     (let* ((length (byte_size msgpack-data))
       ...
       (tuple 'ok (binary (padded-length binary) (msgpack-data binary)))))
    (error error)))
...
(defun pad-to-4-bytes (bin)
  "Pad binary to 4 bytes (big endian)."
  (let ((size (byte_size bin)))
    (case size
      (4 bin)
      (3 (binary (0) (bin binary)))
      (2 (binary (0) (0) (bin binary)))
      (1 (binary (0) (0) (0) (bin binary)))
      (0 (binary (0) (0) (0) (0)))
      (_ bin))))  ;; Should not happen
