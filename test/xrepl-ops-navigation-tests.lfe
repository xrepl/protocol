(defmodule xrepl-ops-navigation-tests
...
(deftest find-definition-request-minimal
  (let ((req (xrepl-ops-navigation:find-definition-request #m(symbol "map:get"))))
...
(deftest find-references-request-minimal
  (let ((req (xrepl-ops-navigation:find-references-request #m(symbol "map:get"))))
    (is-equal #"find_references" (maps:get #"op" req))
    (is-equal #"map:get" (maps:get #"symbol" req))))

(deftest find-references-request-with-location
  (let ((req (xrepl-ops-navigation:find-references-request
              #m(symbol "foo" file "test.lfe" line 10 column 5))))
    (is-equal #"find_references" (maps:get #"op" req))
    (is-equal #"foo" (maps:get #"symbol" req))
    (is-equal #"test.lfe" (maps:get #"file" req))
...
(deftest list-definitions-request-with-file
  (let ((req (xrepl-ops-navigation:list-definitions-request #m(file "src/foo.lfe"))))
...
(deftest list-definitions-request-with-module
  (let ((req (xrepl-ops-navigation:list-definitions-request #m(module "lists"))))
    (is-equal #"list_definitions" (maps:get #"op" req))
    (is-equal #"lists" (maps:get #"module" req))))

(deftest list-definitions-request-with-session
  (let ((req (xrepl-ops-navigation:list-definitions-request
...
(deftest workspace-symbols-request-construction
  (let ((req (xrepl-ops-navigation:workspace-symbols-request #m(query "map"))))
    (is-equal #"workspace_symbols" (maps:get #"op" req))
    (is-equal #"map" (maps:get #"query" req))))

(deftest workspace-symbols-request-with-session
  (let ((req (xrepl-ops-navigation:workspace-symbols-request
...
(deftest parse-find-definition-request
  (let ((msg #m(#"op" #"find_definition" #"symbol" #"map:get")))
    (case (xrepl-ops-navigation:parse-request msg)
      (`#(ok ,parsed)
...
(deftest parse-find-definition-response
  (let* ((locs (list #m(#"file" #"src/foo.lfe" #"line" 42)))
    ...
    (case (xrepl-ops-navigation:parse-response #"find_definition" msg)
      (`#(ok ,result)
...
(deftest valid-request-checks
  (is (xrepl-ops-navigation:valid-request? #m(#"op" #"find_definition" #"symbol" #"foo")))
  (is (xrepl-ops-navigation:valid-request? #m(#"op" #"find_references" #"symbol" #"bar")))
  (is (xrepl-ops-navigation:valid-request? #m(#"op" #"list_definitions")))
  (is (xrepl-ops-navigation:valid-request? #m(#"op" #"symbol_at_point")))
  (is (xrepl-ops-navigation:valid-request? #m(#"op" #"workspace_symbols" #"query" #"map")))
  (is-not (xrepl-ops-navigation:valid-request? #m(#"op" #"unknown")))
  (is-not (xrepl-ops-navigation:valid-request? #m(#"op" #"find_definition")))  ;; missing symbol
  (is-not (xrepl-ops-navigation:valid-request? #m(#"op" #"find_references")))  ;; missing symbol
  (is-not (xrepl-ops-navigation:valid-request? #m(#"op" #"workspace_symbols"))))  ;; missing query

(deftest valid-response-checks
  (is (xrepl-ops-navigation:valid-response? #"find_definition" #m(#"status" #"done")))
  (is (xrepl-ops-navigation:valid-response? #"find_references" #m(#"status" #"done")))
  (is (xrepl-ops-navigation:valid-response? #"workspace_symbols" #m(#"status" #"error")))
  (is-not (xrepl-ops-navigation:valid-response? #"find_definition" #m(#"status" #"invalid"))))
...
(deftest find-definition-round-trip
  (let* ((req (xrepl-ops-navigation:find-definition-request
    ...
    (is (xrepl-ops-navigation:valid-request? decoded))))
