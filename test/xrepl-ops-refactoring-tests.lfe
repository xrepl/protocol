(defmodule xrepl-ops-refactoring-tests
...
(deftest rename-symbol-request-missing-symbol
  (let ((result (xrepl-ops-refactoring:rename-symbol-request #m(new_name "bar"))))
    (case result
...
(deftest rename-symbol-request-missing-new-name
  (let ((result (xrepl-ops-refactoring:rename-symbol-request #m(symbol "foo"))))
    (case result
      (`#(error ,_)
       (is 'true))
      (_
...
(deftest extract-function-request-missing-code
  (let ((result (xrepl-ops-refactoring:extract-function-request
                 #m(function_name "fn"))))
    (case result
      (`#(error missing-code)
       (is 'true))
...
(deftest extract-function-request-missing-function-name
  (let ((result (xrepl-ops-refactoring:extract-function-request #m(code "code"))))
    (case result
...
(deftest inline-function-request-minimal
  (let ((req (xrepl-ops-refactoring:inline-function-request #m(function "add-nums"))))
...
(deftest inline-function-request-with-symbol-alias
  (let ((req (xrepl-ops-refactoring:inline-function-request #m(symbol "foo"))))
    (is-equal #"inline_function" (maps:get #"op" req))
...
(deftest inline-function-request-missing-function
  (let ((result (xrepl-ops-refactoring:inline-function-request #m())))
    (case result
...
(deftest parse-rename-symbol-request
  (let ((msg #m(#"op" #"rename_symbol"
                #"symbol" #"foo"
                #"new_name" #"bar")))
    (case (xrepl-ops-refactoring:parse-request msg)
      (`#(ok ,parsed)
...
(deftest parse-rename-symbol-response
  (let* ((changes (maps:put #"files_changed" 3 #m()))
    ...
    (case (xrepl-ops-refactoring:parse-response msg 'rename_symbol)
      (`#(ok ,parsed)
...
(deftest parse-extract-function-response
  (let* ((result (maps:put #"function" #"(defun foo () ...)" #m()))
    ...
    (case (xrepl-ops-refactoring:parse-response msg 'extract_function)
      (`#(ok ,parsed)
       (is-equal #"done" (maps:get #"status" parsed))
...
(deftest parse-inline-function-response
  (let* ((result (maps:put #"files_changed" 2 #m()))
         (msg (maps:put #"status" #"done" (maps:put #"result" result #m()))))
    (case (xrepl-ops-refactoring:parse-response msg 'inline_function)
      (`#(ok ,parsed)
       (is-equal #"done" (maps:get #"status" parsed))
       (is-equal result (maps:get #"result" parsed)))
      (other
...
(deftest valid-inline-function-request
  (let ((req (xrepl-ops-refactoring:inline-function-request #m(function "foo"))))
    (is (xrepl-ops-refactoring:valid-request? req))))
...
(deftest valid-rename-symbol-response
  (let* ((changes (maps:put #"files_changed" 3 #m()))
         (resp (xrepl-ops-refactoring:rename-symbol-response changes)))
    (is (xrepl-ops-refactoring:valid-response? resp 'rename_symbol))))

(deftest invalid-rename-symbol-response-missing-changes
  (let ((resp #m(#"status" #"done")))
    (is-not (xrepl-ops-refactoring:valid-response? resp 'rename_symbol))))

(deftest valid-extract-function-response
  (let* ((result (maps:put #"function" #"..." #m()))
         (resp (xrepl-ops-refactoring:extract-function-response result)))
    (is (xrepl-ops-refactoring:valid-response? resp 'extract_function))))

(deftest invalid-extract-function-response-missing-result
  (let ((resp #m(#"status" #"done")))
    (is-not (xrepl-ops-refactoring:valid-response? resp 'extract_function))))

(deftest valid-inline-function-response
  (let* ((result (maps:put #"files_changed" 2 #m()))
         (resp (xrepl-ops-refactoring:inline-function-response result)))
    (is (xrepl-ops-refactoring:valid-response? resp 'inline_function))))

(deftest invalid-inline-function-response-missing-result
  (let ((resp #m(#"status" #"done")))
    (is-not (xrepl-ops-refactoring:valid-response? resp 'inline_function))))
...
(deftest error-construction
  (let ((err (xrepl-ops-refactoring:error 'refactoring-failed "Refactoring failed")))
