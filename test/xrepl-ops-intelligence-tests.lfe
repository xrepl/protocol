...
(deftest complete-request-minimal
  (let ((req (xrepl-ops-intelligence:complete-request #m(prefix "map:"))))
...
(deftest complete-context-request-minimal
  (let ((req (xrepl-ops-intelligence:complete-context-request
...
(deftest complete-context-response-has-both-field-names
  (let* ((candidates (list #m(#"text" #"defun")))
         ...
         (resp (xrepl-ops-intelligence:complete-context-response candidates context)))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest signature-request-defaults-to-signature
  (let ((req (xrepl-ops-intelligence:signature-request #m(symbol "map:get"))))
...
(deftest signature-response-with-session
  (let* ((sigs (list #m(#"label" #"(foo)")))
         (resp (xrepl-ops-intelligence:signature-response sigs #m(session "s1"))))
    (is-equal #"done" (maps:get #"status" resp))
    (is-equal #"s1" (maps:get #"session" resp))))
...
(deftest eldoc-request-construction
  (let ((req (xrepl-ops-intelligence:eldoc-request #m(symbol "map:get"))))
    (is-equal #"eldoc" (maps:get #"op" req))
    (is-equal #"map:get" (maps:get #"symbol" req))))

(deftest eldoc-request-with-session
  (let ((req (xrepl-ops-intelligence:eldoc-request #m(symbol "map:get" session "s1"))))
    (is-equal #"eldoc" (maps:get #"op" req))
    (is-equal #"map:get" (maps:get #"symbol" req))
...
(deftest eldoc-response-construction
  (let* ((info #m(#"signature" #"(map:get key map)" #"doc" #"Get value from map"))
         (resp (xrepl-ops-intelligence:eldoc-response info)))
    (is-equal #"done" (maps:get #"status" resp))
    (is-equal info (maps:get #"eldoc" resp))))

(deftest eldoc-response-with-session
  (let* ((info #m(#"signature" #"(foo)"))
         (resp (xrepl-ops-intelligence:eldoc-response info #m(session "s1"))))
    (is-equal #"done" (maps:get #"status" resp))
    (is-equal #"s1" (maps:get #"session" resp))))
...
(deftest eldoc-batch-request-construction
  (let* ((syms (list #"map:get" #"lists:map"))
         (req (xrepl-ops-intelligence:eldoc-batch-request (maps:put 'symbols syms #m()))))
...
(deftest eldoc-batch-response-construction
  (let* ((results (list #m(#"symbol" #"map:get" #"signature" #"(map:get key map)")))
         (resp (xrepl-ops-intelligence:eldoc-batch-response results)))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest type-info-request-construction
  (let ((req (xrepl-ops-intelligence:type-info-request #m(symbol "map"))))
    (is-equal #"type_info" (maps:get #"op" req))
    (is-equal #"map" (maps:get #"symbol" req))))

(deftest type-info-request-with-session
  (let ((req (xrepl-ops-intelligence:type-info-request #m(symbol "map" session "s1"))))
    (is-equal #"type_info" (maps:get #"op" req))
    (is-equal #"s1" (maps:get #"session" req))))

(deftest type-info-response-construction
  (let* ((info #m(#"type" #"map()" #"spec" #"map(any(), any())"))
         (resp (xrepl-ops-intelligence:type-info-response info)))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest format-request-defaults-to-format
  (let ((req (xrepl-ops-intelligence:format-request #m(code "(defun foo())"))))
...
(deftest format-response-construction
  (let ((resp (xrepl-ops-intelligence:format-response "(defun foo\n  ())")))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest indent-info-request-construction
  (let ((req (xrepl-ops-intelligence:indent-info-request #m(line 10))))
    (is-equal #"indent_info" (maps:get #"op" req))
    (is-equal 10 (maps:get #"line" req))))

(deftest indent-info-request-with-session
  (let ((req (xrepl-ops-intelligence:indent-info-request #m(line 10 session "s1"))))
    (is-equal #"indent_info" (maps:get #"op" req))
    (is-equal #"s1" (maps:get #"session" req))))

(deftest indent-info-response-construction
  (let* ((info #m(#"indent" 2 #"method" #"defun"))
         (resp (xrepl-ops-intelligence:indent-info-response info)))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest buffer-analysis-request-minimal
  (let ((req (xrepl-ops-intelligence:buffer-analysis-request #m(code "(defun foo ())"))))
...
(deftest buffer-analysis-response-construction
  (let* ((analysis #m(#"warnings" (list) #"errors" (list) #"functions" 1))
         (resp (xrepl-ops-intelligence:buffer-analysis-response analysis)))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest highlight-regions-request-construction
  (let ((req (xrepl-ops-intelligence:highlight-regions-request #m(code "(defun foo ())"))))
    (is-equal #"highlight_regions" (maps:get #"op" req))
    (is-equal #"(defun foo ())" (maps:get #"code" req))))

(deftest highlight-regions-request-with-session
  (let ((req (xrepl-ops-intelligence:highlight-regions-request
              #m(code "(defun foo ())" session "s1"))))
    (is-equal #"highlight_regions" (maps:get #"op" req))
    (is-equal #"s1" (maps:get #"session" req))))
...
(deftest highlight-regions-response-construction
  (let* ((regions (list #m(#"start" 0 #"end" 5 #"type" #"keyword")))
         (resp (xrepl-ops-intelligence:highlight-regions-response regions)))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest parse-complete-request
  (let ((msg #m(#"op" #"complete" #"prefix" #"map:")))
    (case (xrepl-ops-intelligence:parse-request msg)
      (`#(ok ,parsed)
...
(deftest parse-complete-response
  (let* ((candidates (list #m(#"text" #"map")))
    ...
    (case (xrepl-ops-intelligence:parse-response #"complete" msg)
      (`#(ok ,result)
...
(deftest valid-request-checks
  (is (xrepl-ops-intelligence:valid-request? #m(#"op" #"complete" #"prefix" #"foo")))
  (is (xrepl-ops-intelligence:valid-request? #m(#"op" #"complete_context")))
  (is (xrepl-ops-intelligence:valid-request? #m(#"op" #"signature")))
  (is (xrepl-ops-intelligence:valid-request? #m(#"op" #"signature_help")))
  (is (xrepl-ops-intelligence:valid-request? #m(#"op" #"eldoc")))
  (is (xrepl-ops-intelligence:valid-request? #m(#"op" #"eldoc_batch")))
  (is (xrepl-ops-intelligence:valid-request? #m(#"op" #"type_info")))
  (is (xrepl-ops-intelligence:valid-request? #m(#"op" #"format")))
  (is (xrepl-ops-intelligence:valid-request? #m(#"op" #"format_code")))
  (is (xrepl-ops-intelligence:valid-request? #m(#"op" #"apropos")))
  (is (xrepl-ops-intelligence:valid-request? #m(#"op" #"indent_info")))
  (is (xrepl-ops-intelligence:valid-request? #m(#"op" #"buffer_analysis")))
  (is (xrepl-ops-intelligence:valid-request? #m(#"op" #"highlight_regions")))
  (is-not (xrepl-ops-intelligence:valid-request? #m(#"op" #"unknown")))
  (is-not (xrepl-ops-intelligence:valid-request? #m(#"op" #"complete"))))  ;; missing prefix

(deftest valid-response-checks
  (is (xrepl-ops-intelligence:valid-response? #"complete" #m(#"status" #"done")))
  (is (xrepl-ops-intelligence:valid-response? #"signature" #m(#"status" #"done")))
  (is (xrepl-ops-intelligence:valid-response? #"eldoc" #m(#"status" #"error")))
  (is-not (xrepl-ops-intelligence:valid-response? #"complete" #m(#"status" #"invalid"))))
