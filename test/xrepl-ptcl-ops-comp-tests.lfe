(defmodule xrepl-ptcl-ops-comp-tests
...
(deftest compile-file-request-minimal
  (let ((req (xrepl-ptcl-ops-comp:compile-file-request #m(file "src/foo.lfe"))))
...
(deftest compile-file-request-with-path-alias
  (let ((req (xrepl-ptcl-ops-comp:compile-file-request #m(path "test.lfe"))))
    (is-equal #"compile_file" (maps:get #"op" req))
...
(deftest compile-file-request-with-options
  (let* ((opts-map (maps:put #"warnings" 'true #m()))
         (req (xrepl-ptcl-ops-comp:compile-file-request
...
(deftest compile-file-request-with-session
  (let ((req (xrepl-ptcl-ops-comp:compile-file-request
              #m(file "foo.lfe" session "s1"))))
    (is-equal #"compile_file" (maps:get #"op" req))
    (is-equal #"s1" (maps:get #"session" req))))

(deftest compile-file-request-missing-file
  (let ((result (xrepl-ptcl-ops-comp:compile-file-request #m())))
    (case result
      (`#(error missing-file)
       (is 'true))
      (_
...
(deftest compile-file-response-construction
  (let* ((result (maps:put #"success" 'true
         ...
         (resp (xrepl-ptcl-ops-comp:compile-file-response result)))
...
(deftest compile-project-request-minimal
  (let ((req (xrepl-ptcl-ops-comp:compile-project-request #m())))
    (is-equal #"compile_project" (maps:get #"op" req))))

(deftest compile-project-request-with-path
  (let ((req (xrepl-ptcl-ops-comp:compile-project-request
...
(deftest compile-project-request-with-options
  (let* ((opts-map (maps:put #"verbose" 'true #m()))
         (req (xrepl-ptcl-ops-comp:compile-project-request
               `#m(options ,opts-map))))
    (is-equal #"compile_project" (maps:get #"op" req))
    (is-equal opts-map (maps:get #"options" req))))

(deftest compile-project-request-with-session
  (let ((req (xrepl-ptcl-ops-comp:compile-project-request #m(session "s1"))))
    (is-equal #"compile_project" (maps:get #"op" req))
    (is-equal #"s1" (maps:get #"session" req))))
...
(deftest compile-project-response-construction
  (let* ((result (maps:put #"success" 'true
         ...
         (resp (xrepl-ptcl-ops-comp:compile-project-response result)))
    (is-equal #"done" (maps:get #"status" resp))
    (is-equal result (maps:get #"result" resp))))
...
(deftest lint-response-construction
  (let* ((issues (list (maps:put #"severity" #"warning"
         ...
         (resp (xrepl-ptcl-ops-comp:lint-response issues)))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest dependencies-request-minimal
  (let ((req (xrepl-ptcl-ops-comp:dependencies-request #m())))
    (is-equal #"dependencies" (maps:get #"op" req))))

(deftest dependencies-request-with-path
  (let ((req (xrepl-ptcl-ops-comp:dependencies-request
              #m(path "/path/to/project"))))
    (is-equal #"dependencies" (maps:get #"op" req))
    (is-equal #"/path/to/project" (maps:get #"path" req))))

(deftest dependencies-request-with-type
  (let ((req (xrepl-ptcl-ops-comp:dependencies-request #m(type "direct"))))
    (is-equal #"dependencies" (maps:get #"op" req))
    (is-equal #"direct" (maps:get #"type" req))))

(deftest dependencies-request-with-session
  (let ((req (xrepl-ptcl-ops-comp:dependencies-request #m(session "s1"))))
    (is-equal #"dependencies" (maps:get #"op" req))
    (is-equal #"s1" (maps:get #"session" req))))
...
(deftest dependencies-response-construction
  (let* ((deps (list (maps:put #"name" #"lfe"
         ...
         (resp (xrepl-ptcl-ops-comp:dependencies-response deps)))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest build-request-with-session
  (let ((req (xrepl-ptcl-ops-comp:build-request #m(task "build" session "s1"))))
    (is-equal #"build" (maps:get #"op" req))
    (is-equal #"s1" (maps:get #"session" req))))
...
(deftest build-response-construction
  (let* ((result (maps:put #"success" 'true
         ...
         (resp (xrepl-ptcl-ops-comp:build-response result)))
    (is-equal #"done" (maps:get #"status" resp))
    (is-equal result (maps:get #"result" resp))))
...
(deftest parse-compile-file-request
  (let ((msg #m(#"op" #"compile_file" #"file" #"src/foo.lfe")))
    (case (xrepl-ptcl-ops-comp:parse-request msg)
      (`#(ok ,parsed)
...
(deftest parse-compile-file-response
  (let* ((result (maps:put #"success" 'true #m()))
    ...
    (case (xrepl-ptcl-ops-comp:parse-response msg 'compile_file)
      (`#(ok ,parsed)
...
(deftest parse-compile-project-response
  (let* ((result (maps:put #"files_compiled" 10 #m()))
         (msg (maps:put #"status" #"done" (maps:put #"result" result #m()))))
    (case (xrepl-ptcl-ops-comp:parse-response msg 'compile_project)
      (`#(ok ,parsed)
       (is-equal #"done" (maps:get #"status" parsed))
       (is-equal result (maps:get #"result" parsed)))
      (other
...
(deftest parse-lint-response
  (let* ((issues (list))
    ...
    (case (xrepl-ptcl-ops-comp:parse-response msg 'lint)
      (`#(ok ,parsed)
       (is-equal #"done" (maps:get #"status" parsed))
...
(deftest parse-dependencies-response
  (let* ((deps (list))
    ...
    (case (xrepl-ptcl-ops-comp:parse-response msg 'dependencies)
      (`#(ok ,parsed)
       (is-equal #"done" (maps:get #"status" parsed))
...
(deftest parse-build-response
  (let* ((result (maps:put #"success" 'true #m()))
         (msg (maps:put #"status" #"done" (maps:put #"result" result #m()))))
    (case (xrepl-ptcl-ops-comp:parse-response msg 'build)
      (`#(ok ,parsed)
       (is-equal #"done" (maps:get #"status" parsed))
       (is-equal result (maps:get #"result" parsed)))
      (other
...
(deftest valid-compile-file-request
  (let ((req (xrepl-ptcl-ops-comp:compile-file-request #m(file "foo.lfe"))))
    (is (xrepl-ptcl-ops-comp:valid-request? req))))

(deftest invalid-compile-file-request-missing-file
  (let ((req #m(#"op" #"compile_file")))
    (is-not (xrepl-ptcl-ops-comp:valid-request? req))))
...
(deftest valid-compile-file-response
  (let* ((result (maps:put #"success" 'true #m()))
         (resp (xrepl-ptcl-ops-comp:compile-file-response result)))
    (is (xrepl-ptcl-ops-comp:valid-response? resp 'compile_file))))

(deftest invalid-compile-file-response-missing-result
  (let ((resp #m(#"status" #"done")))
    (is-not (xrepl-ptcl-ops-comp:valid-response? resp 'compile_file))))

(deftest valid-compile-project-response
  (let* ((result (maps:put #"files_compiled" 10 #m()))
         (resp (xrepl-ptcl-ops-comp:compile-project-response result)))
    (is (xrepl-ptcl-ops-comp:valid-response? resp 'compile_project))))

(deftest valid-lint-response
  (let ((resp (xrepl-ptcl-ops-comp:lint-response (list))))
    (is (xrepl-ptcl-ops-comp:valid-response? resp 'lint))))

(deftest invalid-lint-response-missing-issues
  (let ((resp #m(#"status" #"done")))
    (is-not (xrepl-ptcl-ops-comp:valid-response? resp 'lint))))

(deftest valid-dependencies-response
  (let ((resp (xrepl-ptcl-ops-comp:dependencies-response (list))))
    (is (xrepl-ptcl-ops-comp:valid-response? resp 'dependencies))))

(deftest invalid-dependencies-response-missing-deps
  (let ((resp #m(#"status" #"done")))
    (is-not (xrepl-ptcl-ops-comp:valid-response? resp 'dependencies))))

(deftest valid-build-response
  (let* ((result (maps:put #"success" 'true #m()))
         (resp (xrepl-ptcl-ops-comp:build-response result)))
    (is (xrepl-ptcl-ops-comp:valid-response? resp 'build))))

(deftest invalid-build-response-missing-result
  (let ((resp #m(#"status" #"done")))
    (is-not (xrepl-ptcl-ops-comp:valid-response? resp 'build))))
...
(deftest error-construction
  (let ((err (xrepl-ptcl-ops-comp:error 'compilation-failed "Compilation failed")))
