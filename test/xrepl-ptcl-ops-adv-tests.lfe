(defmodule xrepl-ptcl-ops-adv-tests
...
(deftest macroexpand-request-minimal
  (let ((req (xrepl-ptcl-ops-adv:macroexpand-request #m(form "(when true (print 1))"))))
...
(deftest macroexpand-request-with-expand-once
  (let ((req (xrepl-ptcl-ops-adv:macroexpand-request #m(form "(when true 1)" expand_once true))))
    (is-equal #"macroexpand" (maps:get #"op" req))
    (is-equal 'true (maps:get #"expand_once" req))))

(deftest macroexpand-request-with-session
  (let ((req (xrepl-ptcl-ops-adv:macroexpand-request #m(form "(when)" session "sess-123"))))
    (is-equal #"macroexpand" (maps:get #"op" req))
    (is-equal #"sess-123" (maps:get #"session" req))))

(deftest macroexpand-request-missing-form
  (let ((result (xrepl-ptcl-ops-adv:macroexpand-request #m())))
    (is-equal 'error (element 1 result))
    (is-equal 'missing-form (element 2 result))))

(deftest macroexpand-response-construction
  (let ((resp (xrepl-ptcl-ops-adv:macroexpand-response #m(expansion #"(progn (print 1))"))))
...
(deftest macroexpand-all-request-minimal
  (let ((req (xrepl-ptcl-ops-adv:macroexpand-all-request #m(form "(when (unless))"))))
...
(deftest macroexpand-all-request-with-session
  (let ((req (xrepl-ptcl-ops-adv:macroexpand-all-request #m(form "(test)" session "s1"))))
    (is-equal #"macroexpand_all" (maps:get #"op" req))
    (is-equal #"s1" (maps:get #"session" req))))

(deftest macroexpand-all-request-missing-form
  (let ((result (xrepl-ptcl-ops-adv:macroexpand-all-request #m())))
    (is-equal 'error (element 1 result))
    (is-equal 'missing-form (element 2 result))))

(deftest macroexpand-all-response-construction
  (let ((resp (xrepl-ptcl-ops-adv:macroexpand-all-response #m(expansion #"(fully expanded)"))))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest search-history-request-missing-query
  (let ((result (xrepl-ptcl-ops-adv:search-history-request #m())))
    (is-equal 'error (element 1 result))
    (is-equal 'missing-query (element 2 result))))

(deftest search-history-response-construction
  (let ((resp (xrepl-ptcl-ops-adv:search-history-response
...
(deftest profile-start-request-minimal
  (let ((req (xrepl-ptcl-ops-adv:profile-start-request #m())))
    (is-equal #"profile_start" (maps:get #"op" req))))

(deftest profile-start-request-with-target
  (let ((req (xrepl-ptcl-ops-adv:profile-start-request
               #m(target "mymodule"))))
    (is-equal #"profile_start" (maps:get #"op" req))
...
(deftest profile-start-response-construction
  (let ((resp (xrepl-ptcl-ops-adv:profile-start-response #m(profile_id #"prof-123"))))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest profile-stop-request-minimal
  (let ((req (xrepl-ptcl-ops-adv:profile-stop-request #m())))
    (is-equal #"profile_stop" (maps:get #"op" req))))

(deftest profile-stop-request-with-id-and-format
  (let ((req (xrepl-ptcl-ops-adv:profile-stop-request
...
(deftest profile-stop-response-construction
  (let ((resp (xrepl-ptcl-ops-adv:profile-stop-response
...
(deftest benchmark-request-missing-code
  (let ((result (xrepl-ptcl-ops-adv:benchmark-request #m())))
    (is-equal 'error (element 1 result))
    (is-equal 'missing-code (element 2 result))))

(deftest benchmark-response-construction
  (let ((resp (xrepl-ptcl-ops-adv:benchmark-response
...
(deftest snippets-request-minimal
  (let ((req (xrepl-ptcl-ops-adv:snippets-request #m())))
    (is-equal #"snippets" (maps:get #"op" req))))

(deftest snippets-request-with-filters
  (let ((req (xrepl-ptcl-ops-adv:snippets-request
...
(deftest snippets-response-construction
  (let ((resp (xrepl-ptcl-ops-adv:snippets-response
...
(deftest expand-snippet-request-minimal
  (let ((req (xrepl-ptcl-ops-adv:expand-snippet-request #m(id "for-loop"))))
...
(deftest expand-snippet-request-with-params
  (let ((req (xrepl-ptcl-ops-adv:expand-snippet-request
...
(deftest generate-function-request-minimal
  (let ((req (xrepl-ptcl-ops-adv:generate-function-request #m(name "my-func"))))
...
(deftest generate-function-request-with-arity-and-params
  (let ((req (xrepl-ptcl-ops-adv:generate-function-request
...
(deftest generate-function-request-missing-name
  (let ((result (xrepl-ptcl-ops-adv:generate-function-request #m())))
    (is-equal 'error (element 1 result))
    (is-equal 'missing-name (element 2 result))))

(deftest generate-function-response-construction
  (let ((resp (xrepl-ptcl-ops-adv:generate-function-response
...
(deftest suggest-improvements-request-minimal
  (let ((req (xrepl-ptcl-ops-adv:suggest-improvements-request
...
(deftest suggest-improvements-request-missing-code
  (let ((result (xrepl-ptcl-ops-adv:suggest-improvements-request #m())))
    (is-equal 'error (element 1 result))
    (is-equal 'missing-code (element 2 result))))

(deftest suggest-improvements-response-construction
  (let ((resp (xrepl-ptcl-ops-adv:suggest-improvements-response
...
(deftest share-session-request-minimal
  (let ((req (xrepl-ptcl-ops-adv:share-session-request #m())))
    (is-equal #"share_session" (maps:get #"op" req))))

(deftest share-session-request-with-include-and-expiry
  (let ((req (xrepl-ptcl-ops-adv:share-session-request
...
(deftest share-session-response-construction
  (let ((resp (xrepl-ptcl-ops-adv:share-session-response
...
(deftest restore-session-request-minimal
  (let ((req (xrepl-ptcl-ops-adv:restore-session-request #m(share_id "share-abc123"))))
...
(deftest text-document-did-open-request-minimal
  (let ((req (xrepl-ptcl-ops-adv:text-document-did-open-request
...
(deftest text-document-did-open-request-missing-fields
  (let ((result (xrepl-ptcl-ops-adv:text-document-did-open-request
                  #m(uri "test"))))
    (is-equal 'error (element 1 result))
    (is-equal 'missing-required-fields (element 2 result))))

(deftest text-document-did-open-response-construction
  (let ((resp (xrepl-ptcl-ops-adv:text-document-did-open-response #m())))
    (is-equal #"done" (maps:get #"status" resp))))
...
(deftest text-document-did-change-request-minimal
  (let ((req (xrepl-ptcl-ops-adv:text-document-did-change-request
...
(deftest text-document-did-change-request-missing-fields
  (let ((result (xrepl-ptcl-ops-adv:text-document-did-change-request
                  #m(uri "test"))))
    (is-equal 'error (element 1 result))
    (is-equal 'missing-required-fields (element 2 result))))

(deftest text-document-did-change-response-construction
  (let ((resp (xrepl-ptcl-ops-adv:text-document-did-change-response #m())))
    (is-equal #"done" (maps:get #"status" resp))))
...
(deftest parse-macroexpand-request
  (let* ((req #m(op #"macroexpand" form #"(when true 1)"))
         (`#(ok ,parsed) (xrepl-ptcl-ops-adv:parse-request req)))
...
(deftest valid-macroexpand-request-true
  (let ((req #m(op #"macroexpand" form #"(test)")))
    (is-equal 'true (xrepl-ptcl-ops-adv:valid-request? req))))

(deftest valid-macroexpand-request-false-missing-form
  (let ((req #m(op #"macroexpand")))
    (is-equal 'false (xrepl-ptcl-ops-adv:valid-request? req))))
...
(deftest valid-response-true
  (let ((resp #m(status #"done" expansion #"(test)")))
    (is-equal 'true (xrepl-ptcl-ops-adv:valid-response? 'macroexpand resp))))

(deftest valid-response-false-missing-status
  (let ((resp #m(expansion #"(test)")))
    (is-equal 'false (xrepl-ptcl-ops-adv:valid-response? 'macroexpand resp))))
...
(deftest macroexpand-request-msgpack-roundtrip
  (let* ((req (xrepl-ptcl-ops-adv:macroexpand-request #m(form "(when true 1)")))
...
(deftest history-request-msgpack-roundtrip
  (let* ((req (xrepl-ptcl-ops-adv:history-request #m(limit 10 reverse true)))
         (`#(ok ,encoded) (xrepl-ptcl-msgpack:encode req))
         (`#(ok ,decoded) (xrepl-ptcl-msgpack:decode encoded)))
...
(deftest benchmark-request-msgpack-roundtrip
  (let* ((req (xrepl-ptcl-ops-adv:benchmark-request #m(code "(test)" iterations 1000)))
         (`#(ok ,encoded) (xrepl-ptcl-msgpack:encode req))
         (`#(ok ,decoded) (xrepl-ptcl-msgpack:decode encoded)))
...
(deftest macroexpand-response-msgpack-roundtrip
  (let* ((resp (xrepl-ptcl-ops-adv:macroexpand-response #m(expansion #"(expanded)")))
