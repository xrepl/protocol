(defmodule xrepl-ptcl-ops-sess-tests
...
(deftest clone-request-construction
  (let ((req (xrepl-ptcl-ops-sess:clone-request)))
    (is-equal #"clone" (maps:get #"op" req))))

(deftest clone-response-construction
  (let ((resp (xrepl-ptcl-ops-sess:clone-response "new-session-123")))
...
(deftest close-request-no-session
  (let ((req (xrepl-ptcl-ops-sess:close-request)))
    (is-equal #"close" (maps:get #"op" req))))

(deftest close-request-with-session
  (let ((req (xrepl-ptcl-ops-sess:close-request "session-123")))
    (is-equal #"close" (maps:get #"op" req))
    (is-equal #"session-123" (maps:get #"session" req))))

(deftest close-response-construction
  (let ((resp (xrepl-ptcl-ops-sess:close-response)))
    (is-equal #"done" (maps:get #"status" resp))))

;;; ls_sessions tests

(deftest ls-sessions-request-construction
  (let ((req (xrepl-ptcl-ops-sess:ls-sessions-request)))
...
(deftest ls-sessions-response-construction
  (let* ((sessions (list #m(#"id" #"s1" #"active" 'true)
         ...
         (resp (xrepl-ptcl-ops-sess:ls-sessions-response sessions)))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest switch-namespace-request-construction
  (let ((req (xrepl-ptcl-ops-sess:switch-namespace-request #m(namespace "my-module"))))
...
(deftest switch-namespace-request-with-session
  (let ((req (xrepl-ptcl-ops-sess:switch-namespace-request
...
(deftest switch-namespace-response-construction
  (let ((resp (xrepl-ptcl-ops-sess:switch-namespace-response "my-module")))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest session-info-request-construction
  (let ((req (xrepl-ptcl-ops-sess:session-info-request #m(session "abc123"))))
...
(deftest session-info-response-construction
  (let* ((info #m(#"id" #"abc123"
                  #"created" 1234567890
                  #"namespace" #"user"))
         (resp (xrepl-ptcl-ops-sess:session-info-response info)))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest clear-session-request-construction
  (let ((req (xrepl-ptcl-ops-sess:clear-session-request #m(session "abc123"))))
    (is-equal #"clear_session" (maps:get #"op" req))
    (is-equal #"abc123" (maps:get #"session" req))))

(deftest clear-session-response-construction
  (let ((resp (xrepl-ptcl-ops-sess:clear-session-response 'true)))
    (is-equal #"done" (maps:get #"status" resp))
...
(deftest upload-history-request-with-commands
  (let* ((cmds (list "cmd1" "cmd2"))
         ...
         (req (xrepl-ptcl-ops-sess:upload-history-request opts)))
...
(deftest upload-history-response-simple
  (let ((resp (xrepl-ptcl-ops-sess:upload-history-response 5 #m())))
    (is-equal #"done" (maps:get #"status" resp))
    (is-equal 5 (maps:get #"uploaded" resp))))

(deftest upload-history-response-with-session
  (let ((resp (xrepl-ptcl-ops-sess:upload-history-response 5 #m(session "abc123"))))
    (is-equal #"done" (maps:get #"status" resp))
    (is-equal 5 (maps:get #"uploaded" resp))
...
(deftest parse-clone-request
  (let ((msg #m(#"op" #"clone")))
    (case (xrepl-ptcl-ops-sess:parse-request msg)
      (`#(ok ,parsed)
...
(deftest parse-clone-response
  (let ((msg #m(#"status" #"done" #"new_session" #"new-123")))
    (case (xrepl-ptcl-ops-sess:parse-response #"clone" msg)
      (`#(ok ,result)
...
(deftest parse-ls-sessions-response
  (let* ((sessions (list #m(#"id" #"s1")))
    ...
    (case (xrepl-ptcl-ops-sess:parse-response #"ls_sessions" msg)
      (`#(ok ,result)
...
(deftest valid-request-checks
  (is (xrepl-ptcl-ops-sess:valid-request? #m(#"op" #"clone")))
  (is (xrepl-ptcl-ops-sess:valid-request? #m(#"op" #"ls_sessions")))
  (is (xrepl-ptcl-ops-sess:valid-request? #m(#"op" #"upload_history" #"commands" (list "c1"))))
  (is-not (xrepl-ptcl-ops-sess:valid-request? #m(#"op" #"unknown"))))

(deftest valid-response-checks
  (is (xrepl-ptcl-ops-sess:valid-response? #"clone" #m(#"status" #"done" #"new_session" #"s1")))
  (is (xrepl-ptcl-ops-sess:valid-response? #"close" #m(#"status" #"done")))
  (is (xrepl-ptcl-ops-sess:valid-response? #"ls_sessions" #m(#"status" #"done" #"sessions" (list))))
  (is-not (xrepl-ptcl-ops-sess:valid-response? #"clone" #m(#"status" #"invalid"))))
...
(deftest clone-request-round-trip
  (let* ((req (xrepl-ptcl-ops-sess:clone-request))
    ...
    (is (xrepl-ptcl-ops-sess:valid-request? decoded))))
...
(deftest switch-namespace-round-trip
  (let* ((req (xrepl-ptcl-ops-sess:switch-namespace-request #m(namespace "my-mod" session "s1")))
         (`#(ok ,encoded) (xrepl-ptcl-msgpack:encode req))
         (`#(ok ,decoded) (xrepl-ptcl-msgpack:decode encoded)))
