(defmodule xrepl-ops-debugging-tests
...
(deftest set-breakpoint-request-minimal
  (let ((req (xrepl-ops-debugging:set-breakpoint-request #m(file "src/foo.lfe" line 42))))
...
(deftest clear-breakpoint-request-minimal
  (let ((req (xrepl-ops-debugging:clear-breakpoint-request #m(id "bp-1"))))
    (is-equal #"clear_breakpoint" (maps:get #"op" req))
    (is-equal #"bp-1" (maps:get #"id" req))))

(deftest clear-breakpoint-request-with-session
  (let ((req (xrepl-ops-debugging:clear-breakpoint-request #m(id "bp-2" session "s1"))))
    (is-equal #"clear_breakpoint" (maps:get #"op" req))
...
(deftest list-breakpoints-request-minimal
  (let ((req (xrepl-ops-debugging:list-breakpoints-request #m())))
...
(deftest stacktrace-request-minimal
  (let ((req (xrepl-ops-debugging:stacktrace-request #m())))
...
(deftest inspect-locals-request-minimal
  (let ((req (xrepl-ops-debugging:inspect-locals-request #m())))
    (is-equal #"inspect_locals" (maps:get #"op" req))))

(deftest inspect-locals-request-with-frame
  (let ((req (xrepl-ops-debugging:inspect-locals-request #m(frame 2))))
    (is-equal #"inspect_locals" (maps:get #"op" req))
...
(deftest eval-in-frame-request-minimal
  (let ((req (xrepl-ops-debugging:eval-in-frame-request #m(code "x + y"))))
    (is-equal #"eval_in_frame" (maps:get #"op" req))
    (is-equal #"x + y" (maps:get #"code" req))))

(deftest eval-in-frame-request-with-frame
  (let ((req (xrepl-ops-debugging:eval-in-frame-request #m(code "foo(x)" frame 2))))
    (is-equal #"eval_in_frame" (maps:get #"op" req))
...
(deftest step-request-minimal
  (let ((req (xrepl-ops-debugging:step-request #m())))
    (is-equal #"step" (maps:get #"op" req))))

(deftest step-request-with-type
  (let ((req (xrepl-ops-debugging:step-request #m(type "into"))))
    (is-equal #"step" (maps:get #"op" req))
    (is-equal #"into" (maps:get #"type" req))))

(deftest step-request-with-session
  (let ((req (xrepl-ops-debugging:step-request #m(type "over" session "s1"))))
    (is-equal #"step" (maps:get #"op" req))
...
(deftest parse-set-breakpoint-request
  (let ((msg #m(#"op" #"set_breakpoint" #"file" #"src/foo.lfe" #"line" 42)))
    (case (xrepl-ops-debugging:parse-request msg)
      (`#(ok ,parsed)
...
(deftest parse-set-breakpoint-response
  (let* ((bp #m(#"id" #"bp-1" #"file" #"src/foo.lfe" #"line" 42))
    ...
    (case (xrepl-ops-debugging:parse-response #"set_breakpoint" msg)
      (`#(ok ,result)
...
(deftest valid-request-checks
  (is (xrepl-ops-debugging:valid-request? #m(#"op" #"set_breakpoint" #"file" #"foo.lfe" #"line" 42)))
  (is (xrepl-ops-debugging:valid-request? #m(#"op" #"clear_breakpoint" #"id" #"bp-1")))
  (is (xrepl-ops-debugging:valid-request? #m(#"op" #"list_breakpoints")))
  (is (xrepl-ops-debugging:valid-request? #m(#"op" #"stacktrace")))
  (is (xrepl-ops-debugging:valid-request? #m(#"op" #"inspect_locals")))
  (is (xrepl-ops-debugging:valid-request? #m(#"op" #"eval_in_frame" #"code" #"x")))
  (is (xrepl-ops-debugging:valid-request? #m(#"op" #"step")))
  (is-not (xrepl-ops-debugging:valid-request? #m(#"op" #"unknown")))
  (is-not (xrepl-ops-debugging:valid-request? #m(#"op" #"set_breakpoint" #"line" 42)))  ;; missing file
  (is-not (xrepl-ops-debugging:valid-request? #m(#"op" #"clear_breakpoint")))  ;; missing id
  (is-not (xrepl-ops-debugging:valid-request? #m(#"op" #"eval_in_frame"))))  ;; missing code

(deftest valid-response-checks
  (is (xrepl-ops-debugging:valid-response? #"set_breakpoint" #m(#"status" #"done")))
  (is (xrepl-ops-debugging:valid-response? #"stacktrace" #m(#"status" #"done")))
  (is (xrepl-ops-debugging:valid-response? #"step" #m(#"status" #"error")))
  (is-not (xrepl-ops-debugging:valid-response? #"set_breakpoint" #m(#"status" #"invalid"))))
